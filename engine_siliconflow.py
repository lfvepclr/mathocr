...
_ANNOT_TEXT_RE = re.compile(r"<\|TEXT_START\|>(.*?)<\|TEXT_END\|>", re.S)
_LOC_BLOCK_RE = re.compile(r"<\|LOC_BEGIN\|>(.*?)<\|LOC_END\|>", re.S)
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            if exc.code != 429 and 400 <= exc.code < 500:
                raise EngineError(
                    f"硅基流动接口返回 {exc.code}: {detail or exc.reason}"
                ) from exc
...
def process_pages_iter(page_images: list[str], cfg: dict, on_usage=None):
    ...
    logger.info(
        ...
        len(page_images), cfg["model"], workers,
    )

    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as pool:
        futures = [
            pool.submit(_process_one_page, img, cfg) for img in page_images
        ]
        try:
