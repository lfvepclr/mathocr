...
# Image resolution
# ---------------------------------------------------------------------------
def _resolve_image_src(src: str, batch_id: str, file_id=None,
                       page_id=None) -> Path | None:
    ...
    if file_id is not None and page_id is not None:
        if "/" not in src:
            return (BATCHES_DIR / batch_id / "results" / file_id
                    / f"page_{page_id}_images" / src)
        # Old local batches prefix table images with "imgs/" though files
        # are stored flat — resolve those by bare file name.
        candidate = (BATCHES_DIR / batch_id / "results" / file_id
                     / f"page_{page_id}_images" / Path(src).name)
        if candidate.exists():
            return candidate
...
# ---------------------------------------------------------------------------
_CENTER_DIV_RE = re.compile(
    r"^<div[^>]*text-align:\s*center[^>]*>(.*)</div>$", re.DOTALL)
...
def _img_src_to_data_uri(src: str, batch_id: str, file_id=None,
...
def _embed_html_images(html_text: str, batch_id: str, file_id=None,
                       page_id=None) -> str:
...
def _md_to_word_html(md_text: str, batch_id: str, file_id=None,
                     page_id=None) -> str:
    ...
    while i < len(lines):
        ...
        if m:
            ...
            clean = html.escape(
                _convert_latex_in_text(_strip_html_tags(content).strip()))
        ...
        if cm:
            out.append(
                f'<p align="center">{_escape_keep_imgs(cm.group(1).strip())}</p>')
            i += 1
            continue
        ...
        while i + 1 < len(lines):
            ...
            if (not nxt or nxt.startswith("#") or nxt.startswith("|")
                    or nxt.startswith("<table") or "<img" in nxt
...
def _block_fragment(blocks: list, idx: int, md_text: str) -> str:
    ...
    if "<img" not in fragment and label in _IMAGE_LABELS:
        ...
        order = sum(1 for b in blocks[:idx + 1]
                    if (b.get("block_label") or "") in _IMAGE_LABELS) - 1
...
def page_to_richtext(batch_id: str, file_id: str, page_id: int,
                     block_idx: int | None = None,
                     block_idxs: list | None = None) -> dict:
    ...
    md_text = (md_path.read_text(encoding="utf-8")
               if md_path and md_path.exists() else "")
    ...
    for block in blocks:
        ...
        if label in _IMAGE_LABELS:
            ...
            if tag:
                ...
                if src_m:
                    uri = _img_src_to_data_uri(
                        src_m.group(1), batch_id, file_id, page_id)
            if not uri:
                continue
            ...
            body = (f'<div class="t t-{label}">'
                    f'{_inline_text_to_html(_strip_html_tags(content))}</div>')
            cls = "lb"
        else:
