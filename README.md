...
| 前端框架 | React 19 + TypeScript | 模块化、职责单一;组件化拆分 |
| 前端构建 | Vite + Bun | Vite 打包 (输出到 static/),Bun 管理依赖 |
| 前端状态 | zustand | 轻量状态管理 (appStore/viewerStore/queueStore) |
| 前端渲染 | marked + KaTeX | Markdown + LaTeX 公式渲染 (npm 依赖,Vite 自动打包字体) |
| 后端结构 | app/ 包 (分层架构) | api/services/engines/core/utils 分层,server.py 为薄 shim |
| 包管理 | UV (Python) + Bun (前端) | Python 3.13 (paddle 无 cp314 wheel,暂不升 3.14) |
...
# 4. 安装其他依赖 (pyproject.toml 声明)
uv pip install -r pyproject.toml

# 5. 安装前端依赖并构建 (React + Vite)
...
cd web
bun install
bun run dev  # Vite dev server (proxy /api → localhost:7861)
# 或: bun run build  # 构建到 ../static/
```
...
# 代码质量检查(Ruff lint/format + ty 类型检查,提交前跑一遍)
uvx ruff check .
uvx ruff format .
uvx ty check
```
