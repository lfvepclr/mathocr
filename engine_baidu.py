...
MAX_PDF_PAGES = 500
SUPPORTED_SUFFIXES = {
    ".pdf", ".jpg", ".jpeg", ".png", ".bmp", ".tif", ".tiff", ".ofd",
}
...
def _get_access_token(api_key: str, secret_key: str) -> str:
    ...
    if stored and stored_exp:
        try:
            if datetime.fromisoformat(stored_exp) > now:
                _token_cache.update(
                    token=stored, expire_at=datetime.fromisoformat(stored_exp)
                )
                return stored
        except ValueError:
            pass
    ...
    try:
        with urllib.request.urlopen(
            f"{TOKEN_URL}?{params}", timeout=HTTP_TIMEOUT
        ) as resp:
    ...
    if not token:
        raise EngineError(
            f"百度 access_token 获取失败: {data.get('error_description') or data}"
        )
...
def _post_form(url: str, token: str, fields: dict) -> dict:
    ...
    if data.get("error_code"):
        raise EngineError(
            f"百度接口错误 {data['error_code']}: {data.get('error_msg', '')}"
        )
    return data
...
def _validate_input(file_path: Path):
    ...
    if suffix == ".pdf":
        if size > MAX_DOC_BYTES:
            raise EngineError(
                f"PDF 超过百度接口上限 100MB (当前 {size / 1024 / 1024:.1f}MB)"
            )
        import pdf_renderer
        ...
        raise EngineError(
            f"图片超过百度接口上限 10MB (当前 {size / 1024 / 1024:.1f}MB)"
        )
...
def _title_level(layout: dict, label: str) -> int:
    ...
    if level <= 2 and re.match(r"^\s*\d+\s*[.、]", str(layout.get("text") or "")):
        level = 3
    logger.debug("baidu title: sub_type=%r level=%d text=%.20s",
                 sub_type, level, str(layout.get("text") or ""))
    return level
...
def _adapt_page(page: dict, page_idx: int) -> dict:
    ...
    tables_by_layout = {
        t.get("layout_id"): t for t in (page.get("tables") or [])
    }
    images_by_layout = {
        im.get("layout_id"): im for im in (page.get("images") or [])
    }
    ...
    for layout in page.get("layouts") or []:
        ...
        if baidu_type == "table":
            ...
            content = _localize_table_images(
                table.get("markdown") or content, images, page_idx
            )
            md_piece = content
            ...
            if content:
                ...
                md_piece = ('<div style="text-align: center;">'
                            f'<img src="{img_name}"{pct}></div>')
            else:
                md_piece = ""
            ...
            content = content.strip()
            md_piece = (
                content if not content or "$" in content
                else f"${content}$"
            )
            ...
            md_piece = (f'<div style="text-align: center;">{content}</div>'
                        if content else "")
...
def process_file_iter(file_path: str, cfg: dict, on_usage=None):
    ...
    logger.info("Baidu doc-parse: submitting %s (%.1fMB)",
                path.name, path.stat().st_size / 1024 / 1024)
    submitted = _post_form(SUBMIT_URL, token, {
        "file_data": file_data,
        "file_name": path.name,
        "analysis_chart": "true",
        "merge_tables": "true",
    ...
    logger.info("Baidu doc-parse: %d page(s) returned for %s",
                len(pages), path.name)
