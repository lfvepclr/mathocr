...
from app.engines import local as ocr_engine
from app.engines import registry as engine_registry
from app.services import image_annotator, pdf_renderer, settings_store

logger = logging.getLogger(__name__)

# ---------------------------------------------------------------------------
# Paths (centralised in app.config so relocations never break)
# ---------------------------------------------------------------------------
    ...
    for table, columns in _EXPECTED_COLUMNS.items():
        ...
        for col, ddl in columns.items():
            if col not in existing:
                ...
                logger.info("DB migration: added column %s.%s (%s)", table, col, ddl)
    conn.commit()
...
def _natural_sort_key(name: str) -> list:
    return [int(t) if t.isdigit() else t.lower() for t in _NATURAL_SORT_RE.split(name)]


def create_batch(uploaded_files: list[tuple[str, bytes]], engine: str = "local") -> str:
    ...
    with _db_lock:
        db = _get_db()
        db.execute(
            "INSERT INTO batches (batch_id, status, file_count, engine) VALUES (?, ?, ?, ?)",
    ...
    logger.info("Created batch %s with %d files (engine=%s)", batch_id, len(file_records), engine)
    return batch_id
...
def get_batch_live_progress(batch_id: str) -> dict:
    ...
    if cur:
        ...
        cur_done = sum(1 for p in get_pages(batch_id, cur["file_id"]) if p["has_result"])
...
def list_batches(limit: int = 50, offset: int = 0, status: str | None = None) -> list[dict]:
    with _db_lock:
        db = _get_db()
        if status:
            rows = db.execute(
    ...
    with _db_lock:
        ...
        db.execute(
            """UPDATE batches SET
               ...
               WHERE batch_id = ?""",
            [calls, prompt_tokens, completion_tokens, billed_pages, cost, batch_id],
        )
        db.commit()
        row = db.execute(
...
def reset_interrupted_tasks() -> int:
    ...
    with _db_lock:
        db = _get_db()
        cur = db.execute(
            "UPDATE batches SET status = 'queued', processing_time = 0 WHERE status = 'processing'"
        )
...
def get_queue_status_map(limit: int = 50) -> dict[str, str]:
    ...
    with _db_lock:
        rows = (
            _get_db()
            .execute(
                "SELECT batch_id, status FROM batches ORDER BY created_at DESC LIMIT ?",
                [limit],
            )
            .fetchall()
        )
...
def count_queued_batches() -> int:
    ...
    with _db_lock:
        row = _get_db().execute("SELECT COUNT(*) FROM batches WHERE status = 'queued'").fetchone()
    ...
    return [
        {
            ...
            "markdown_path": r[4],
            "json_path": r[5],
            "original_image_path": r[6],
            "annotated_image_path": r[7],
            "images_dir": r[8],
            "processing_time": r[9],
        }
        for r in rows
...
def get_page(batch_id: str, file_id: str, page_id: int) -> dict | None:
    ...
    return {
        "page_id": row[0],
        "has_result": bool(row[1]),
        "block_count": row[2],
        "avg_score": row[3],
        "markdown_path": row[4],
        "json_path": row[5],
        "original_image_path": row[6],
        "annotated_image_path": row[7],
        "images_dir": row[8],
...
def process_batch_background(batch_id: str):
    ...
    try:
        ...
        if not files:
            ...
            event_bus.publish(
                batch_id, "batch_completed", {"status": "completed", "processing_time": 0}
            )
            return
        ...
        logger.info(
            "Processing batch %s sequentially with engine '%s' (%d/%d files pending)",
            batch_id,
            engine,
            len(pending_files),
            len(files),
        ...
        for file_info in pending_files:
            try:
                ...
                update_file_status(
                    batch_id, file_info["file_id"], "error", error_message=str(exc)[:500]
                )
...
def _process_single_file(batch_id: str, file_info: dict, engine: str = "local"):
    ...
    event_bus.publish(
        batch_id, "file_started", {"file_id": file_id, "original_name": original_name}
    )
    ...
    if prev_total > 0:
        ...
        if len(done_pages) >= prev_total and all(
            p["markdown_path"] and Path(p["markdown_path"]).exists() for p in done_pages
        ):
            logger.info(
                "File %s already fully processed (%d pages), skipping", original_name, prev_total
            )
    ...
    logger.info(
        "Starting OCR for %s (%d pages, engine=%s) — this may take a while",
        original_name,
        total_pages,
        engine,
    )
    event_bus.publish(
        ...
        "page_started",
        {
            ...
            "avg_page_time": round(avg_page_time, 1),
            "engine": engine,
            "stage": stage,
        },
    )
...
def _process_single_page(
    ...
    for block in page_result["json_data"].get("res", {}).get("parsing_res_list") or []:
        content = block.get("block_content")
        if content and 'src="imgs/' in content:
            block["block_content"] = re.sub(r'src="imgs/([^"]+)"', r'src="\1"', content)
    ...
    for img_name, pil_img in images.items():
        ...
        if basename != img_name:
            markdown_text = markdown_text.replace(f'src="{basename}"', f'src="{api_url}"')
    ...
    original_img = (
        original_images[page_idx] if page_idx < len(original_images) else original_images[0]
    )
    try:
        image_annotator.annotate_image(original_img, page_result["page_data"], annotated_path)
    except Exception:
