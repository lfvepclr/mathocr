...
def _vlm_server_reachable(url: str, timeout: float = 0.8) -> bool:
    ...
    for path in ("/health", "/v1/models", "/"):
        try:
            ...
            continue  # probe endpoint unreachable — try the next one
    return False
...
def _vlm_server_model_id(url: str, timeout: float = 2.0) -> str | None:
    ...
    try:
        ...
        pass  # probe failed — caller falls back to the next model source
    return None
    ...
    if _pipeline is None:
        with _pipeline_lock:
            if _pipeline is None:
                ...
                vl_server_url = os.environ.get("OCR_VL_REC_SERVER_URL", "http://localhost:8111/")
    ...
    *,
    ...
    if engine == engine_registry.LOCAL:
        if not engine_registry.local_runtime_available():
            raise RuntimeError(
                "本地引擎不可用:未安装 paddleocr/paddle,请改用在线引擎或使用 start.sh 完整安装"
            )
    ...
    if engine == engine_registry.SILICONFLOW:
        from app.engines import siliconflow as engine_siliconflow
...
def process_document(
    file_path: str, *, engine: str = "local", page_images: list[str] | None = None
) -> list[dict[str, Any]]:
    """
    ...
    """
    return [
        page for _, page in process_document_iter(file_path, engine=engine, page_images=page_images)
    ]
