...
_LEFT_RIGHT_RE = re.compile(
    r"\\(?:left|right|middle|big|Big|bigg|Bigg|bigl|bigr|Bigl|Bigr|bigm|Bigm|biggl|biggr|Biggl|Biggr)\s*"
)
_SPACING_RE = re.compile(
    r"\\(?:,|;|:|!|>|quad|qquad|enspace|thinspace|medspace|thickspace|negthinspace|displaystyle|textstyle|scriptstyle|scriptscriptstyle|limits|nolimits|displaystyle)\b"
)
...
def _unknown_arg_sub(m: re.Match) -> str:
    ...
    if name not in _unknown_commands:
        _unknown_commands.add(name)
        logger.warning(
            "latex_to_unicode: unmapped LaTeX command '\\%s{...}' -> '%s(...)'", name, name
        )
    return f"{name}({arg.strip()})"
...
def scan_directory(root: str) -> None:
    ...
    print(
        f"Coverage: {cov_total}/{total} occurrences ({100 * cov_total / max(total, 1):.1f}%), "
