...
def set(key: str, value: str):  # noqa: A001 - deliberate KV-store naming
    with _lock:
        db = _db()
        db.execute(
    ...
    with _lock:
        ...
        db.execute(
            """INSERT INTO api_usage
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            [datetime.now().isoformat(), batch_id, file_id, page_id, engine,
...
_AGG_COLS = (
    "SUM(calls), SUM(prompt_tokens), SUM(completion_tokens), "
    "SUM(billed_pages), SUM(cost)"
)
...
def aggregate(scope: str = "all") -> dict:
    ...
    with _lock:
        ...
        total = db.execute(
            f"SELECT {_AGG_COLS} FROM api_usage {clause}"
        ).fetchone()
    return {
        "scope": scope,
...
def aggregate_batch(batch_id: str) -> dict:
    ...
    with _lock:
        row = _db().execute(
            f"SELECT {_AGG_COLS} FROM api_usage WHERE batch_id = ?", [batch_id]
        ).fetchone()
    return _agg_row(row)
