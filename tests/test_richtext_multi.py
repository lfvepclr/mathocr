...
class MultiBlockBaiduTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rt = exporter.page_to_richtext(
            BAIDU_BATCH, BAIDU_FILE, 0, block_idxs=[17, 14, 9])
    ...
    def test_dedupe(self):
        rt = exporter.page_to_richtext(
            BAIDU_BATCH, BAIDU_FILE, 0, block_idxs=[9, 9, 10])
...
class MultiBlockSiliconflowTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rt = exporter.page_to_richtext(
            SF_BATCH, SF_FILE, 0, block_idxs=[15, 16, 1])
...
class ErrorCasesTest(unittest.TestCase):
    def test_out_of_bounds(self):
        ...
        with self.assertRaises(ValueError):
            exporter.page_to_richtext(
                BAIDU_BATCH, BAIDU_FILE, 0, block_idxs=[999])

    def test_empty_set(self):
        _skip_if_missing(BAIDU_BATCH)
        with self.assertRaises(ValueError):
            exporter.page_to_richtext(
                BAIDU_BATCH, BAIDU_FILE, 0, block_idxs=[])
